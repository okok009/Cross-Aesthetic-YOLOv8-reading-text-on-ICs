...
for i in range(len(ann['annotations'])):
    ...
    if img_name not in image_id:
        if ann['annotations'][i]['aesthetic'][2] == 1:
            broken_image_id.append(img_name)
        image_id.append(img_name)

broken_image_file = 'broken_img.txt'
with open(broken_image_file, 'a+') as f:
    for i in range(len(broken_image_id)):
        f.write(broken_image_id[i] + '\n')
