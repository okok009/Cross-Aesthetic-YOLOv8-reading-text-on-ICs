...
class v8ISWLoss:
    ...
    def __call__(self, preds, batch):
        ...
        trans_preds = self.model(trans_img)
        isw_loss = self.isw(preds, trans_preds, self.sigma)
        isw_loss = isw_loss.mean()

        return isw_loss * batch_size, isw_loss.expand(3).detach()

    def isw(self, ori_output, trans_output, sigma=1.2):
        '''
    ...
    def var_matrix(self, cov_1, cov_2):
        ...
        cov_1 = cov_1 - mean
        cov_2 = cov_2 - mean
        variance = (torch.matmul(cov_1, cov_1) + torch.matmul(cov_2, cov_2)) / 2
...
class v8OBB_ISWLoss(v8DetectionLoss):
...
class v8OBB_CRLoss(v8DetectionLoss):
